...
def compare_results(table1, table2, threshold=0.05):
    ...
    pylab.savefig('comp_result1.png')
    if rooted:
        pylab.figure()
        ...
        pylab.savefig('comp_result2.png')
    pylab.show()
...
def hierarchical_clustering(mat, method='average', cluster_distance=True, labels=None, thres=0.65):
    ...
    pylab.savefig('hierach_cluster.png')
    pylab.show()
    return Z1
...
class Preprocess(object):
    ...
    def show_statistics(self):
        ...
        pylab.savefig('show_stat_figure2.png')
        pylab.figure()
        ...
        pylab.savefig('show_stat_figure3.png')
        pylab.figure()
        ...
        pylab.savefig('show_stat_figure4.png')
        ...
        pylab.savefig('show_stat_figure5.png')
        pylab.show()
...
class TopologicalRepresentation(object):
    ...
    def __init__(self, table, lens='mds', metric='correlation', precomputed=False, **kwargs):
        ...
        pylab.savefig('topologial_rep.png')
        pylab.show()
...
class RootedGraph(UnrootedGraph):
    ...
    def dendritic_graph(self):
        ...
        for n in range(diam-1):
            nodedist = []
            for k in self.pl:
                if self.gl.has_node(self.root) and self.gl.has_node(k):
    ...
    def draw_skeleton(self, color, labels=False, ccmap='jet', weight=8.0, save='', ignore_log=False, markpath=False):
        ...
        if labels:
            networkx.draw_networkx_labels(pg, pos, font_size=5, font_family='sans-serif')
        if save == '':
            pylab.savefig('topo_skeloton.png')
            pylab.show()
        else:
            fig.savefig(save)
        return values
    ...
    def plot_CDR_correlation(self, doplot=True):
        ...
        if doplot:
            ...
            pylab.savefig('CDR_correlation.png')
            pylab.show()
        return po
    ...
    def plot_rootlane_correlation(self):
        ...
        pylab.savefig('rootlane_correlation.png')
        pylab.show()
        return self.po
    ...
    def cellular_subpopulations(self, min_dispersion, threshold=0.05, min_cells=5, max_K=8, method='centroid',
        ...
        if method == 'centroid':
            ...
            pylab.savefig('cell_subpop1.png')
            pylab.figure()
            ...
            pylab.savefig('cell_subpop2.png')
            pylab.show()
